...
@Query(value="SELECT COALESCE(SUM(spend),0), COALESCE(SUM(revenue),0), COALESCE(SUM(impressions),0), COALESCE(SUM(clicks),0), COALESCE(SUM(conversions),0) FROM campaign_metrics WHERE business_id=:businessId AND recorded_at>=:since", nativeQuery=true)
List<Object[]> aggregateByBusinessId(@Param("businessId") UUID businessId,@Param("since") Instant since);
}
