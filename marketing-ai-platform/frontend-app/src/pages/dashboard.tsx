...
import { DollarSign, BarChart3, Trophy, AlertCircle, RefreshCw, LayoutDashboard } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">{data.businessName} &middot; Last {data.days} days</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        ...
        <KpiCard label="Winners" value={creativeHealth.winners} trend={creativeHealth.winners > 0 ? 'up' : 'flat'} />
          ...
          {healthData.length === 0 ? (
            ...
            <div className="flex items-center gap-6">
              <div className="h-48 w-48 shrink-0">
                <ResponsiveContainer>
                  <PieChart>
                    <Pie data={healthData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={70} innerRadius={42} strokeWidth={2} stroke="#fff">
                      {healthData.map((_, i) => (
                        <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />
                      ))}
                    </Pie>
                    <Tooltip contentStyle={{ borderRadius: '0.5rem', fontSize: '0.875rem', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }} />
                  </PieChart>
                </ResponsiveContainer>
              </div>
              <div className="space-y-2.5 text-sm">
                {healthData.map((d, i) => (
                  <div key={d.name} className="flex items-center gap-2">
                    <span className="h-2.5 w-2.5 rounded-full ring-2 ring-white" style={{ backgroundColor: PIE_COLORS[i % PIE_COLORS.length] }} />
          ...
          <dl className="space-y-3 text-sm">
            {topSignals.bestPlatform && (
              <div className="flex justify-between rounded-lg bg-gray-50 px-3 py-2">
                ...
                <dd className="font-semibold">{topSignals.bestPlatform}</dd>
              </div>
            )}
            {topSignals.bestAssetType && (
              <div className="flex justify-between rounded-lg bg-gray-50 px-3 py-2">
                <dt className="text-gray-500">Best Asset Type</dt>
                <dd className="font-semibold">{topSignals.bestAssetType}</dd>
              </div>
            )}
            {topSignals.topHook && (
              <div className="flex justify-between rounded-lg bg-gray-50 px-3 py-2">
                <dt className="text-gray-500">Top Hook</dt>
                <dd className="font-semibold max-w-[200px] truncate" title={topSignals.topHook}>{topSignals.topHook}</dd>
              </div>
            )}
          </dl>
        </SectionCard>
      ...
      {syncStatus.length > 0 && (
        <SectionCard title="Platform Connections">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {syncStatus.map((s) => (
              <div key={s.connectionId} className="flex items-center justify-between rounded-lg border border-border p-3 transition-colors hover:bg-gray-50/60">
                <div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-3 gap-3 text-center text-sm">
                <div className="rounded-xl bg-emerald-50 p-3 ring-1 ring-inset ring-emerald-600/10">
                  <p className="text-2xl font-bold tabular-nums text-emerald-600">{outcomeStats.positive}</p>
                  <p className="text-xs text-gray-500">Positive</p>
                </div>
                <div className="rounded-xl bg-red-50 p-3 ring-1 ring-inset ring-red-600/10">
                  <p className="text-2xl font-bold tabular-nums text-red-600">{outcomeStats.negative}</p>
                  <p className="text-xs text-gray-500">Negative</p>
                </div>
                <div className="rounded-xl bg-gray-50 p-3 ring-1 ring-inset ring-gray-500/10">
                  <p className="text-2xl font-bold tabular-nums text-gray-600">{outcomeStats.neutral}</p>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between rounded-lg bg-gray-50 px-3 py-2 text-sm">
                  ...
                  <span className="font-semibold tabular-nums">{outcomeStats.successRate.toFixed(1)}%</span>
                </div>
                <div className="flex items-center justify-between rounded-lg bg-gray-50 px-3 py-2 text-sm">
                  <span className="text-gray-500">Avg Impact</span>
                  <span className="font-semibold tabular-nums">{outcomeStats.avgImpactScore.toFixed(3)}</span>
                </div>
                {outcomeStats.pending > 0 && (
                  <div className="flex items-center justify-between rounded-lg bg-amber-50 px-3 py-2 text-sm">
                    <span className="text-gray-500">Pending</span>
                    <span className="font-semibold text-amber-600 tabular-nums">{outcomeStats.pending}</span>
                  </div>
                )}
              </div>
              <button
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-6">
                <div className="relative h-24 w-24 shrink-0">
                  <svg viewBox="0 0 36 36" className="h-24 w-24 -rotate-90">
                    ...
                    <circle
                      ...
                      stroke={freshness.freshnessScore >= 50 ? '#10b981' : freshness.freshnessScore >= 25 ? '#f59e0b' : '#ef4444'}
                      strokeWidth="2.5" strokeDasharray={`${freshness.freshnessScore}, 100`}
                      strokeLinecap="round"
                ...
                <div className="space-y-1.5">
                  <p className="text-xs font-medium uppercase tracking-wider text-gray-400">Recommended Action</p>
              ...
              {freshness.stalenessSignals && freshness.stalenessSignals.length > 0 && (
                <div className="space-y-1.5 rounded-lg bg-orange-50/60 p-3">
                  <p className="text-xs font-semibold uppercase tracking-wider text-orange-600">Staleness Signals</p>
                  {freshness.stalenessSignals.map((sig: string, i: number) => (
                    <p key={i} className="text-xs text-orange-700">• {sig}</p>
                  ))}
                </div>
              )}
              <button
