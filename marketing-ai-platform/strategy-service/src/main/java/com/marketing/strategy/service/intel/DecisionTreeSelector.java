...
public class DecisionTreeSelector {
  public record SelectionResult(String templateKey, List<Map<String, Object>> decisionPath, String motion, String priceTier, String budgetTier) {}

  public SelectionResult select(Map<String, Object> business, String objective, BigDecimal monthlyBudget) {
    ...
    if ("OFFLINE".equals(motion)) {
      if ("HIGH".equals(priceTier)) key = "OFFLINE_HIGH_TICKET_TRUST_GOOGLE_LOCAL";
      else if ("LOW".equals(priceTier) || "awareness".equals(objectiveNorm) || "traffic".equals(objectiveNorm)) key = "OFFLINE_LOW_TICKET_IMPULSE_PROMO";
      else key = "OFFLINE_MID_TICKET_FESTIVAL_FOOTFALL";
    } else {
      if ("leads".equals(objectiveNorm) || "HIGH".equals(priceTier)) key = "ONLINE_HIGH_TICKET_LEADGEN_APPOINTMENT";
      else if ("LOW".equals(priceTier) || "awareness".equals(objectiveNorm) || "traffic".equals(objectiveNorm)) key = "ONLINE_LOW_TICKET_UGC_BUNDLES";
  ...
  private String inferPriceTier(String raw) {
    ...
    if (raw.contains("premium") || raw.contains("lux") || raw.contains("diamond") || raw.contains("high")) return "HIGH";
    return "MID";
  }
}
