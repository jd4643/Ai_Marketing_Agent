...
public class StrategyService {
    ...
    public String buildPerfSummary(List<Map<String, Object>> metrics, List<Map<String, Object>> winners) {
    ...
    /*private StrategyResponse toResponse(Map<String, Object> m) {
        return new StrategyResponse(UUID.fromString((String) m.get("requestId")), "v1", om.convertValue(m.get("platformBudgetSplit"), new TypeReference<>() {
        }), om.convertValue(m.get("campaignPlan"), new TypeReference<>() {
        }), (String) m.get("funnelStrategy"), (String) m.get("expectedCPL"), (String) m.get("expectedROAS"), (String) m.get("reasoning"), om.convertValue(m.get("assumptions"), new TypeReference<>() {
        }));
    }*/
    private StrategyResponse toResponse(Map<String, Object> m) {
        Object rawId = m.get("requestId");
        String idStr = rawId != null ? rawId.toString() : null;
        UUID requestId;
        if (idStr == null || idStr.isBlank()) {
            // Fallback: generate a new requestId when missing from LLM/fallback payload
            requestId = UUID.randomUUID();
        } else {
            try {
                requestId = UUID.fromString(idStr);
            } catch (IllegalArgumentException ex) {
                // If the string is not a valid UUID, also fallback to a new one
                log.warn("Invalid requestId '{}' in strategy response payload, generating a new UUID", idStr);
                requestId = UUID.randomUUID();
            }
        }
    ...
    private Map<String, Object> getBusiness(UUID id) {
        try (Connection c = ds.getConnection(); PreparedStatement ps = c.prepareStatement("SELECT business_name,industry,product,target_audience FROM business_profile WHERE id=?")) {
            ps.setObject(1, id);
            ResultSet rs = ps.executeQuery();
            if (rs.next())
                return Map.of("businessName", rs.getString(1), "industry", rs.getString(2), "product", rs.getString(3), "targetAudience", rs.getString(4));
        } catch (Exception e) {
    ...
    private List<Map<String, Object>> metrics(UUID id) {
        ...
        try (Connection c = ds.getConnection(); PreparedStatement ps = c.prepareStatement("SELECT platform,COALESCE(SUM(spend),0),COALESCE(AVG(roas),0) FROM campaign_metrics WHERE business_id=? AND recorded_at>=? GROUP BY platform")) {
            ps.setObject(1, id);
            ps.setTimestamp(2, Timestamp.from(Instant.now().minus(30, ChronoUnit.DAYS)));
            ResultSet rs = ps.executeQuery();
            while (rs.next())
                out.add(Map.of("platform", rs.getString(1), "spend", rs.getBigDecimal(2), "avgRoas", rs.getBigDecimal(3)));
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
        return out;
    ...
    private void saveHistory(UUID requestId, StrategyRequest req, String prompt, Map<String, Object> response, String status, String code, String error, Long latency) {
        String sql = """
                INSERT INTO strategy_history(
                    id,request_id,business_id,objective,monthly_budget,trends_json,
                    prompt_version,model_name,request_json,response_json,status,
                    error_code,error_message,openai_latency_ms,created_at
