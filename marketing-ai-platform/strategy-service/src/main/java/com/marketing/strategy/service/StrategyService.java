...
public class StrategyService {
    ...
    private StrategyResponse toResponse(Map<String, Object> m) {
        ...
        String version = Optional.ofNullable(m.get("strategyVersion"))
                ...
                .filter(v -> !v.isBlank())
                .orElse("v1");
        return new StrategyResponse(responseRequestId, version, om.convertValue(m.get("platformBudgetSplit"), new TypeReference<>() {
    ...
    private Map<String, Object> getBusiness(UUID id) {
        try (Connection c = ds.getConnection(); PreparedStatement ps = c.prepareStatement("SELECT business_name,industry,product,target_audience,website_url,price_range FROM business_profile WHERE id=?")) {
            ps.setObject(1, id);
